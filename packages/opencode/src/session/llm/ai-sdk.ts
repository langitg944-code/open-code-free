  ...
  switch (event.type) {
    ...
    case "finish-step":
      ...
      return Effect.sync(() => {
        ...
        state.copilotTotalNanoAiu = undefined
        // AI SDK v6 doesn't always populate finishReason from custom providers.
        // Default to "stop" when undefined (models without tool support always stop).
        const resolvedFinishReason = event.finishReason ?? "stop"
        return [
          LLMEvent.stepFinish({
            index: state.step++,
            reason: finishReason(resolvedFinishReason),
