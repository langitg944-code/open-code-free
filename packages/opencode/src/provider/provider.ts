...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "xemoz-rest": () =>
    import("@opencode-ai/core/plugin/provider/xemoz-rest-client").then((m) => {
      return (opts: any) => ({
        languageModel: (modelId: string) => m.createXemozRest(opts).languageModel(modelId),
        chat: (modelId: string) => m.createXemozRest(opts).languageModel(modelId),
      })
    }),
}
